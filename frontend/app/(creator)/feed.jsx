...
const DEMO_POSTS = [
  ...
  {
    id: 'demo-6',
    title: 'Behind the lens: shooting my own brand content',
    body: 'No agency. No budget. Just an iPhone 14 and a lot of learning. Here is the exact workflow I use to create content that lands brand deals.',
    content_type: 'video_embed',
        ...
        {FILTERS.map(f => {
          ...
          return (
            <TouchableOpacity key={f} style={[s.chip, active && s.chipActive]} onPress={() => setFilter(f)} activeOpacity={0.8}>
              {active
                ? <LinearGradient colors={GRAD_HERO} style={s.chipGrad} start={{ x: 0, y: 0 }} end={{ x: 1, y: 0 }}>
